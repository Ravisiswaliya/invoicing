...
from django.db.models import F, ExpressionWrapper, DecimalField, Sum, Value, Prefetch
...
from sales.serializers import (
    ...
    QuotationSerializer,
    InvoiceFromQuotationSerializer,
    BillSerializer,
)
...
class BaseTransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return self.queryset.select_related("party").prefetch_related(
            Prefetch("products", queryset=ProductDetail.objects.all())
        )


class QuotationViewSet(BaseTransactionViewSet):
...
class BillViewSet(BaseTransactionViewSet):
    ...
    def from_quotation(self, request):
        serializer = InvoiceFromQuotationSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)

        quotation = QuotationDetail.objects.get(
    ...
    def pdf(self, request):
        data = request.data
        bill_no = data["billno"].replace("/", "-")
        file_name = f"INV_{bill_no}.pdf"
        pdf_path = generate_pdf(data, file_name, doc_type="invoice")
